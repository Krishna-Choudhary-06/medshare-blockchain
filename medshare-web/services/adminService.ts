  ...
  return useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        console.warn('Backend unavailable, falling back to mock data for adminDashboard');
        return {
          stats: {
            totalPatients: 1250,
            totalDoctors: 340,
  ...
  return useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        console.warn('Backend unavailable, falling back to mock data for systemHealth');
        return {
          overallScore: 98,
          metrics: {
            cpu: 45,
  ...
  return useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        console.warn('Backend unavailable, falling back to mock data for adminUsersList');
        return [
